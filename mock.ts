export const mockData = [
  {
    name: "My pop list!",
    recommendations: [
      { artist: "Pharrell Williams", song: "Happy" },
      { artist: "Justin Timberlake", song: "Can't Stop the Feeling!" },
      { artist: "Mark Ronson ft. Bruno Mars", song: "Uptown Funk" },
      { artist: "Pharrell Williams", song: "Happy" },
      { artist: "Justin Timberlake", song: "Can't Stop the Feeling!" },
      { artist: "Mark Ronson ft. Bruno Mars", song: "Uptown Funk" },
      { artist: "Pharrell Williams", song: "Happy" },
      { artist: "Justin Timberlake", song: "Can't Stop the Feeling!" },
      { artist: "Mark Ronson ft. Bruno Mars", song: "Uptown Funk" },
      { artist: "Pharrell Williams", song: "Happy" },
      { artist: "Justin Timberlake", song: "Can't Stop the Feeling!" },
      { artist: "Mark Ronson ft. Bruno Mars", song: "Uptown Funk" },
    ],
  },
  {
    name: "My rock list!",
    recommendations: [
      { artist: "Queen", song: "We Will Rock You" },
      { artist: "Bon Jovi", song: "Livin' on a Prayer" },
    ],
  },
  {
    name: "My jazz list!",
    recommendations: [
      { artist: "Miles Davis", song: "So What" },
      { artist: "John Coltrane", song: "My Favorite Things" },
      { artist: "Billie Holiday", song: "Blue Moon" },
      { artist: "Herbie Hancock", song: "Cantaloupe Island" },
    ],
  },
  {
    name: "My hip-hop list!",
    recommendations: [
      { artist: "Eminem", song: "Lose Yourself" },
      { artist: "Kanye West", song: "Stronger" },
      { artist: "Kendrick Lamar", song: "HUMBLE." },
      { artist: "Drake", song: "God's Plan" },
      { artist: "Jay-Z", song: "99 Problems" },
    ],
  },
  {
    name: "My country list!",
    recommendations: [
      { artist: "Johnny Cash", song: "Ring of Fire" },
      { artist: "Dolly Parton", song: "Jolene" },
      { artist: "Garth Brooks", song: "Friends in Low Places" },
      { artist: "Shania Twain", song: "Man! I Feel Like a Woman!" },
    ],
  },
  {
    name: "My EDM list!",
    recommendations: [
      { artist: "Avicii", song: "Levels" },
      { artist: "Martin Garrix", song: "Animals" },
    ],
  },
  {
    name: "My classical list!",
    recommendations: [
      { artist: "Ludwig van Beethoven", song: "Symphony No. 9" },
      { artist: "Wolfgang Amadeus Mozart", song: "Eine kleine Nachtmusik" },
      { artist: "Johann Sebastian Bach", song: "Toccata and Fugue in D minor" },
    ],
  },
  {
    name: "My indie list!",
    recommendations: [
      { artist: "Arctic Monkeys", song: "Do I Wanna Know?" },
      { artist: "The 1975", song: "Somebody Else" },
      { artist: "Tame Impala", song: "The Less I Know the Better" },
      { artist: "Vampire Weekend", song: "A-Punk" },
    ],
  },
  {
    name: "My metal list!",
    recommendations: [
      { artist: "Metallica", song: "Enter Sandman" },
      { artist: "Iron Maiden", song: "The Trooper" },
    ],
  },
  {
    name: "My R&B list!",
    recommendations: [
      { artist: "Beyoncé", song: "Halo" },
      { artist: "Usher", song: "Yeah!" },
      { artist: "Alicia Keys", song: "Fallin'" },
    ],
  }, {
    name: "My pop list!",
    recommendations: [
      { artist: "Pharrell Williams", song: "Happy" },
      { artist: "Justin Timberlake", song: "Can't Stop the Feeling!" },
      { artist: "Mark Ronson ft. Bruno Mars", song: "Uptown Funk" },
    ],
  },
  {
    name: "My rock list!",
    recommendations: [
      { artist: "Queen", song: "We Will Rock You" },
      { artist: "Bon Jovi", song: "Livin' on a Prayer" },
    ],
  },
  {
    name: "My jazz list!",
    recommendations: [
      { artist: "Miles Davis", song: "So What" },
      { artist: "John Coltrane", song: "My Favorite Things" },
      { artist: "Billie Holiday", song: "Blue Moon" },
      { artist: "Herbie Hancock", song: "Cantaloupe Island" },
    ],
  },
  {
    name: "My hip-hop list!",
    recommendations: [
      { artist: "Eminem", song: "Lose Yourself" },
      { artist: "Kanye West", song: "Stronger" },
      { artist: "Kendrick Lamar", song: "HUMBLE." },
      { artist: "Drake", song: "God's Plan" },
      { artist: "Jay-Z", song: "99 Problems" },
    ],
  },
  {
    name: "My country list!",
    recommendations: [
      { artist: "Johnny Cash", song: "Ring of Fire" },
      { artist: "Dolly Parton", song: "Jolene" },
      { artist: "Garth Brooks", song: "Friends in Low Places" },
      { artist: "Shania Twain", song: "Man! I Feel Like a Woman!" },
    ],
  },
  {
    name: "My EDM list!",
    recommendations: [
      { artist: "Avicii", song: "Levels" },
      { artist: "Martin Garrix", song: "Animals" },
    ],
  },
  {
    name: "My classical list!",
    recommendations: [
      { artist: "Ludwig van Beethoven", song: "Symphony No. 9" },
      { artist: "Wolfgang Amadeus Mozart", song: "Eine kleine Nachtmusik" },
      { artist: "Johann Sebastian Bach", song: "Toccata and Fugue in D minor" },
    ],
  },
  {
    name: "My indie list!",
    recommendations: [
      { artist: "Arctic Monkeys", song: "Do I Wanna Know?" },
      { artist: "The 1975", song: "Somebody Else" },
      { artist: "Tame Impala", song: "The Less I Know the Better" },
      { artist: "Vampire Weekend", song: "A-Punk" },
    ],
  },
  {
    name: "My metal list!",
    recommendations: [
      { artist: "Metallica", song: "Enter Sandman" },
      { artist: "Iron Maiden", song: "The Trooper" },
    ],
  },
  {
    name: "My R&B list!",
    recommendations: [
      { artist: "Beyoncé", song: "Halo" },
      { artist: "Usher", song: "Yeah!" },
      { artist: "Alicia Keys", song: "Fallin'" },
    ],
  },
];
